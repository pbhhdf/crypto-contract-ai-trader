...
def main() -> int:
    ...
    symbol = os.getenv("TRADER_LIVE_HANDOFF_SYMBOL", "BTCUSDT").upper()
    try:
        payload = request_json("/api/live-ops-handoff", {"symbol": symbol})
    except HTTPError as exc:
        return fail(f"HTTP {exc.code}: {exc.read().decode('utf-8', errors='replace')}")
    except (URLError, TimeoutError, OSError) as exc:
        return fail(f"could not reach {BASE_URL}: {exc}")

    handoff = payload.get("live_ops_handoff") or payload
    if not isinstance(handoff, dict):
        return fail("live-ops-handoff endpoint did not return an object")
    slug = utc_slug()
    json_path = REPORT_DIR / f"live-ops-handoff-{slug}.json"
    md_path = REPORT_DIR / f"live-ops-handoff-{slug}.md"
    json_path.write_text(json.dumps(handoff, ensure_ascii=False, indent=2), encoding="utf-8")
    md_path.write_text(str(handoff.get("markdown") or json.dumps(handoff, ensure_ascii=False, indent=2)), encoding="utf-8")
    print(
        json.dumps(
            {
                "ok": True,
