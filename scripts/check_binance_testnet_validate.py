...
RUN_TIMEOUT_SECONDS = float(os.getenv("TRADER_RUN_TIMEOUT_SECONDS", "75"))
...
def main() -> int:
    try:
        state = request_json("GET", "/api/state")
        enabled_modes = ((state.get("config") or {}).get("enabled_modes") or [])
        exchange = ((state.get("config") or {}).get("exchange") or {})
        if "binance_testnet_validate" not in enabled_modes:
            return fail("Binance testnet validation mode is not enabled. Set ENABLE_BINANCE_TESTNET=true with testnet keys.")
        if exchange.get("testnet_places_real_orders"):
            return fail("BINANCE_PLACE_TESTNET_ORDERS must be false for this validation check.")
        ...
        try:
            created = request_json("POST", "/api/runs", {"symbol": SYMBOL, "mode": "binance_testnet_validate"})
            ...
            if not orders:
                return fail("testnet validation run completed without an order; final action was HOLD or risk rejected it")
            order = orders[0]
            if order.get("status") != "testnet_validated":
                return fail(f"expected testnet_validated order, got {order.get('status')!r}")
            if order.get("reconcile_status") != "validated_no_live_order":
                return fail(f"expected validated_no_live_order reconcile status, got {order.get('reconcile_status')!r}")
            if order.get("venue_status") != "ORDER_TEST_ACCEPTED":
                return fail(f"expected ORDER_TEST_ACCEPTED venue status, got {order.get('venue_status')!r}")
            ...
            output = {
                ...
                "note": "Binance /fapi/v1/order/test accepted parameters; no real testnet order was placed.",
            }
