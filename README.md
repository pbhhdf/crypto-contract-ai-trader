...
- Basic Auth now rate-limits failed login attempts before the high-permission operator console can be reached; both function-level and temporary HTTP-server smoke tests cover the `401 -> 429 + Retry-After` lockout path, and server templates expose `AUTH_FAILURE_LIMIT`, `AUTH_FAILURE_WINDOW_SECONDS`, and `AUTH_LOCKOUT_SECONDS`.
...
- Unknown Binance protection-order submit states are also tracked as cancelable child OMS records with `UNKNOWN / needs_reconcile`, so the protection-failure guard can cancel or reconcile possible orphan stop-loss/take-profit orders by `clientOrderId`.
- When one Binance stop-loss or take-profit protection order fills, the OMS now cancels the sibling protection order from both REST reconcile and private-stream paths, while zero-fill cancellations leave the remaining protection intact.
- Filled/canceled Binance protection orders now keep protection-specific local statuses such as `testnet_protection_filled`, avoiding confusion with entry-order fills during audit and postflight review.
- When a protection order fills, the parent workflow is marked `testnet_protected_exit` / `live_protected_exit` with a reconciled audit transition so later stateful executions are not blocked by a stale entry-order status.
